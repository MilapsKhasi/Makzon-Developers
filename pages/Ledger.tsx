...
const Ledger = () => {
  ...
  return (
    <div className="p-2 space-y-6">
      ...
      {/* Main Ledger Book View (The UI you requested) */}
      <div className="bg-white border border-slate-300 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between bg-slate-50 p-6 border-b border-slate-200">
          ...
          <div className="flex gap-2">
            <button className="p-2 hover:bg-slate-200 rounded-lg text-slate-400"><Search size={18}/></button>
            <button className="p-2 hover:bg-slate-200 rounded-lg text-slate-400"><Filter size={18}/></button>
          </div>
        </div>
          ...
          <div>
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {/* Filler Rows */}
                {[...Array(Math.max(0, 12 - entries.filter(e => e.type === 'IN').length))].map((_, i) => (
                  <tr key={i} className="divide-x divide-slate-50 h-9"><td/><td/><td/></tr>
                ))}
              </tbody>
            </table>
          </div>
          ...
          <div>
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {[...Array(Math.max(0, 12 - entries.filter(e => e.type === 'OUT').length))].map((_, i) => (
                  <tr key={i} className="divide-x divide-slate-50 h-9"><td/><td/><td/></tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="New Ledger Entry">
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="flex bg-slate-100 p-1 rounded-lg">
             <button type="button" onClick={() => setFormData({...formData, type: 'IN'})} className={`flex-1 py-2 text-[10px] font-bold rounded ${formData.type === 'IN' ? 'bg-white text-green-600 shadow-sm' : 'text-slate-400'}`}>INCOME</button>
             <button type="button" onClick={() => setFormData({...formData, type: 'OUT'})} className={`flex-1 py-2 text-[10px] font-bold rounded ${formData.type === 'OUT' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400'}`}>EXPENSE</button>
          </div>
          <input required type="number" placeholder="0.00" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl font-mono font-bold text-lg text-center" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} />
          <input required type="text" placeholder="PARTICULARS" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold uppercase" value={formData.particulars} onChange={e => setFormData({...formData, particulars: e.target.value})} />
          <div className="grid grid-cols-2 gap-3">
             <input type="date" className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
             <input type="text" placeholder="REF NO" className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs uppercase" value={formData.reference_no} onChange={e => setFormData({...formData, reference_no: e.target.value})} />
          </div>
          <button type="submit" disabled={loading} className="w-full bg-[#FCD34D] py-4 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg active:scale-95 transition-all">
