  ...
  const recalculate = (state: any, sourceField?: string, sourceDutyId?: string, sourceVal?: any) => {
    ...
    if (sourceField === 'total_without_gst') {
      ...
      const items = (state.items || []).map((item: any) => {
        ...
        const itemSubtotal = taxable; // Subtotal is only taxable value without GST
                        ...
                        {formData.items.map((it: any, idx: number) => {
                            ...
                            return (
                                <tr key={it.id}>
                                    ...
                                    <td className="p-0 border-r border-slate-100 dark:border-slate-800"><input type="text" value={toDisplayValue(it.qty)} onChange={e => updateItemRow(idx, 'qty', e.target.value)} placeholder="10,000" className="w-full h-10 px-2 text-center outline-none bg-transparent font-mono font-bold dark:text-white" /></td>
